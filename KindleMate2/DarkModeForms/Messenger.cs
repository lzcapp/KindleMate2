...
namespace KindleMate2.DarkModeForms {
    ...
    public static class Messenger {
        ...
        public static DialogResult MessageBox(string Message) {
            return MessageBox(Message, string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }
        ...
        public static DialogResult MessageBox(string Message, string title, MessageBoxButtons buttons = MessageBoxButtons.OK, MessageBoxIcon icon = MessageBoxIcon.Information) {
        ...
        /// <param name="buttons">One of the MessageBoxButtons values that specifies which buttons to display in the message box.</param>
        /// <returns>It is a modal window, blocking other actions in the application until the user closes it.</returns>
        public static DialogResult MessageBox(string Message, string title, MsgIcon Icon, MessageBoxButtons buttons = MessageBoxButtons.OK) {
            var form = new Form {
            ...
            var bottomPanel = new Panel {
                Dock = DockStyle.Bottom, Height = 48, BackColor = DMode.OScolors.Surface, ForeColor = DMode.OScolors.TextActive
            };
            ...
            List<Button> CmdButtons = new();
            switch (buttons) {
                case MessageBoxButtons.OK:
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.OK, Text = ButtonTranslations["OK"]
                    });
                ...
                case MessageBoxButtons.OKCancel:
                    ...
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.Cancel, Text = ButtonTranslations["Cancel"]
                    });
                    form.AcceptButton = CmdButtons[0];
                ...
                case MessageBoxButtons.AbortRetryIgnore:
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.Retry, Text = ButtonTranslations["Retry"]
                    });
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.Abort, Text = ButtonTranslations["Abort"]
                    });
                    form.AcceptButton = CmdButtons[0];
                    form.CancelButton = CmdButtons[1];
                    break;
                case MessageBoxButtons.YesNoCancel:
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.Yes, Text = ButtonTranslations["Yes"]
                    });
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.No, Text = ButtonTranslations["No"]
                    });
                    CmdButtons.Add(new Button() {
                        Anchor = AnchorStyles.Top | AnchorStyles.Right, DialogResult = DialogResult.Cancel, Text = ButtonTranslations["Cancel"]
                    });
            ...
            foreach (Button _button in CmdButtons) {
            ...
            if (Icon != MsgIcon.None) {
                var picIcon = new PictureBox() {
                    SizeMode = PictureBoxSizeMode.Zoom, Size = new Size(64, 64)
                };
        ...
        public static DialogResult InputBox(string title, string promptText, ref List<KeyValue> Fields, MsgIcon Icon = 0, MessageBoxButtons buttons = MessageBoxButtons.OK) {
            var form = new Form {
                FormBorderStyle = FormBorderStyle.FixedDialog,
                StartPosition = FormStartPosition.CenterParent,
                MaximizeBox = false,
            ...
            if (Icon != MsgIcon.None) {
                var picIcon = new PictureBox() {
                    SizeMode = PictureBoxSizeMode.Zoom, Size = new Size(48, 48)
                };
                picIcon.Image = _Icons.GetIcon(Icon);
            ...
            foreach (KeyValue field in Fields) {
                ...
                var field_label = new Label {
                    Text = field.Key, AutoSize = false, Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleCenter
                };
                ...
                if (field.ValueType == ValueTypes.String) {
                    field_Control = new TextBox {
                        Text = field.Value, Dock = DockStyle.Fill, TextAlign = HorizontalAlignment.Center
                    };
                ...
                if (field.ValueType == ValueTypes.Password) {
                    field_Control = new TextBox {
                        Text = field.Value, Dock = DockStyle.Fill, UseSystemPasswordChar = true, TextAlign = HorizontalAlignment.Center
                    };
                    ((TextBox)field_Control).TextChanged += (sender, args) => {
                        AddTextChangedDelay((TextBox)field_Control, ChangeDelayMS, text => {
                            field.Value = ((TextBox)sender).Text;
                ...
                if (field.ValueType == ValueTypes.Time) {
                    field_Control = new DateTimePicker {
                        Value = Convert.ToDateTime(field.Value), Dock = DockStyle.Fill, Format = DateTimePickerFormat.Time
                    };
                    ((DateTimePicker)field_Control).ValueChanged += (sender, args) => {
                        field.Value = ((DateTimePicker)sender).Value.ToString();
                        //aqui 'KeyValue' valida el nuevo valor y puede cancelarlo
                ...
                if (field.ValueType == ValueTypes.Boolean) {
                    field_Control = new CheckBox {
                        Checked = Convert.ToBoolean(field.Value), Dock = DockStyle.Fill, Text = field.Key
                    };
        ...
        public static string GetCurrentLanguage(string pDefault = "en") {
            ...
            if (IsCurrentLanguageSupported(new List<string>() {
                "en",
                "es",
                "fr",
                "de",
        ...
        private static Dictionary<string, string> GetButtonTranslations(string pLanguage) {
            ...
            Dictionary<string, string> ButtonTranslations = new() {
                {
    ...
    public class KeyValue {
        ...
        protected virtual void OnValidate(ref string newValue) {
            ...
            if (validateHandler != null) {
                //1. We Raize the 'Validate' Event to the Client informing both the
                //	 New and Old Values for Client Side Validation:
                var args = new ValidateEventArgs(newValue) {
                    OldValue = _value
                };
    ...
    public class Base64Icons {
        ...
        public Image GetIcon(string pName) {
            ...
            if (_Icons != null && _Icons.Count > 0) {
                Base64Image? Found = _Icons.Find(x => x.Name == pName);
                if (Found != null) {
                    _ret = Found.Image;
                }
            }
        ...
        public Image GetIcon(MsgIcon pIcon) {
            return GetIcon(pIcon.ToString());
        }
